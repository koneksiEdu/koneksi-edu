  ...
  <div v-if="isOpen" class="z-20 fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center">
    <div class="bg-white p-6 rounded-lg shadow-lg max-w-60 w-full max-h-[70vh] overflow-y-auto">
      <div class="flex justify-between items-center mb-4">
        <h2 class="text-2xl my-text font-semibold">Keranjang</h2>
      ...
      <div v-if="cartItems.length > 0" class="cart-items space-y-4">
        <div v-for="(item, index) in cartItems" :key="index" class="cart-item bg-gray-100 p-4 rounded-lg shadow-md animate__animated animate__fadeIn">
          <div class="item-info mb-2">
            <h3 class="text-md my-text font-semibold">{{ item.judul }}</h3>
          ...
          <div class="item-controls flex items-center justify-between">
            <div class="shadow-md flex rounded=md bg-gray-300 items-center space-x-2">
              <button @click="decrementItem(index)" class="my-text2 font-bold text-white rounded-md w-8 h-8 flex items-center justify-center focus:outline-none">
                <
              </button>
              <span class="my-text">{{ item.jumlah }}</span>
              <button @click="incrementItem(index)" class="my-text2 font-bold text-white rounded-md w-8 h-8 flex items-center justify-center focus:outline-none">
                >
              </button>
            </div>
            <button @click="removeItem(index)" class="myBtn text-white rounded-md w-8 h-8 flex items-center justify-center focus:outline-none">
        ...
        <div class="cart-total mt-6 flex flex-col items-center">
          ...
          <button @click="checkout" class="myBtn mt-2 text-white px-4 py-2 rounded-lg focus:outline-none animate__animated animate__pulse">
            Checkout
          </button>
        </div>
      </div>
